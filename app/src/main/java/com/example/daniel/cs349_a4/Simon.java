...
class Simon {
    ...
    protected ArrayList<Integer> sequence = new ArrayList<Integer>();
    protected int index;

    protected boolean debug;

    protected void init(int _buttons, boolean _debug){
    ...
    public void newRound() {
        ...
        if (debug) {
            System.out.println("[DEBUG] newRound, Simon::state "
                    + getStateAsString());
        }
    ...
    int nextButton() {
        ...
        if (state != State.COMPUTER) {
            System.out.println("[WARNING] nextButton called in "
                    +getStateAsString());
            return -1;
        }
        ...
        if (debug) {
            System.out.println("[DEBUG] nextButton:  index " + index
                    + " button " + button);
        }
    ...
    boolean verifyButton(int button) {
        ...
        if (state != State.HUMAN) {
            System.out.println("[WARNING] verifyButton called in "
                    + getStateAsString());
            return false;
        }
        ...
        if (!correct) {
            ...
            if (index == sequence.size()) {
                ...
                if (debug) {
                    System.out.println("[DEBUG] state is now " + getStateAsString());
                    System.out.println("[DEBUG] new score " + score
